...
export class PublicationStore {
  ...
  doUpdateBooleanField = _.throttle(async <K extends keyof this>(field: K) => {
    await this.editPublication({
      [field]: this[field]
    });
    this.savingStatus = SavingStatusState.SAVED;
  ...
  invertBoolean<K extends keyof this>(field: K): void {
    ...
    this.savingStatus = SavingStatusState.SAVING;
    void this.doUpdateBooleanField(field);
  };
    ...
    method
      .then(
        action((response) => {
          ...
          if (publication.previouslyPublishedDatasetData?.length) {
            this.previouslyPublishedDatasetData = publication.previouslyPublishedDatasetData ?? '';
          }
          if (publication.replicationOfPreviousExperimentsData?.length) {
            this.replicationOfPreviousExperimentsData = publication.replicationOfPreviousExperimentsData ?? '';
          }
          this.isNegative = publication.isNegative;
          this.isPreviouslyPublishedDataset = publication.isPreviouslyPublishedDataset;
          this.isReplicationOfPreviousExperiments = publication.isReplicationOfPreviousExperiments;
